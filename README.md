This fork is for small customizations that are helpful for the Great Big team. So far, the only change is automatically adding the Asana ID to task descriptions when they are toggled in Asana. We use this in a Toggl -> Asana integration to tie the Toggl task back to the Asana task.

Note: Toggling on the "Board" view in Asana doesn't currently add the task ID to the description but all other views work.
